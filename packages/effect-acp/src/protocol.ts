...
export const makeAcpPatchedProtocol = Effect.fn("makeAcpPatchedProtocol")(function* (
  ...
  const ensureActive = Ref.get(terminalCause).pipe(
    Effect.flatMap((cause) => (cause === undefined ? Effect.void : Effect.failCause(cause))),
  );
  ...
  const offerOutgoing = Effect.fn("offerOutgoing")(function* (
    ...
    // ACP has no RPC interrupt method.
    if (message._tag === "Interrupt") return;
    yield* ensureActive;
      ...
      const encodedRequestId =
        message._tag === "Request"
          ...
          : "requestId" in message
            ...
            : undefined;
      const requestId =
        encodedRequestId === "" || encodedRequestId === undefined
          ? undefined
