...
An empty `Claude config directory` means T3 Code uses Claude Code's normal config directory.
...
macOS Keychain, and running it with an artificial home can cause Keychain errors. Use
`Claude config directory` instead.
...
## Legacy Claude Config Directory

`Legacy Claude config directory` remains available for existing provider instances. It is an older
alias for `CLAUDE_CONFIG_DIR`; new instances should use `Claude config directory`.
