...
import { DesktopSnapShotId, EnvironmentId, ThreadId } from "@t3tools/contracts";
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const target = {
          ...
          threadId: ThreadId.make("notification-thread"),
        };
        yield* desktopWindow.dispatchNotificationClick(target);
        assert.equal(foreground, "T3 Code");
        assert.deepEqual(operations, ["send", "reveal", "send", "send", "send", "reveal"]);
        assert.deepEqual(fakeWindow.send.mock.calls.at(-1), [NOTIFICATION_CLICKED_CHANNEL, target]);
      }).pipe(Effect.provide(layer));
    }),
  );
