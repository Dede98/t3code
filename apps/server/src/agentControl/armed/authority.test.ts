...
it.live("keeps unowned candidate execution history fatal", () =>
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
    yield* runMigrations();
    const sequence = yield* seedArmedSource(sql);
    yield* insertCandidate(sql, sequence);
    yield* sql`INSERT INTO agent_control_events (
      event_id, aggregate_kind, stream_id, stream_version, event_type, occurred_at,
      command_id, causation_event_id, correlation_id, actor_authority, payload_json, metadata_json
    ) VALUES ('unowned-stage-event', 'stage-run', 'unowned-stage', 1,
      'agentControl.stageRun.prepared', ${at}, 'unowned-stage-command', NULL,
      'unowned-stage-command', 'controller', ${canonicalJson({ projectId, taskId: "armed-task-1" })}, '{"schemaVersion":1}')`;
    const failure = yield* claimArmedDispatch(sql, {
      projectId,
      ownerId: "owner",
      claimedAt: later,
      expiresAt,
    }).pipe(Effect.flip);
    assert.equal(failure.reason, "task-history-corrupt");
    assert.deepEqual(
      yield* sql`SELECT count(*) AS count FROM agent_control_armed_dispatch_evidence`,
      [{ count: 0 }],
    );
  }).pipe(Effect.provide(NodeSqliteClient.layerMemory())),
);
