...
const make = Effect.gen(function* () {
  ...
  const getMainCursor = sql<{ readonly lastAppliedSequence: unknown }>`
    ...
    WHERE projector_name = ${AGENT_CONTROL_STAGE_RUN_LEASE_PROJECTOR}
  `.pipe(
    Effect.mapError(
      (cause) =>
        new AgentControlPersistenceSqlError({
          operation: "AgentControlStageRunLeaseProjection.mainCursor:get",
          cause,
        }),
    ),
    Effect.flatMap((rows) => {
    ...
    (expectedSequence === 0
      ? sql<{ readonly projectorName: unknown }>`
          INSERT INTO main.agent_control_projection_state (
            ...
            ${AGENT_CONTROL_STAGE_RUN_LEASE_PROJECTOR}, ${sequence}, ${occurredAt}
          )
      ...
      Effect.mapError(
        (cause) =>
          new AgentControlPersistenceSqlError({
            operation: "AgentControlStageRunLeaseProjection.mainCursor:advance",
            cause,
          }),
      ),
