...
layer("Agent Control stage-run lease foundation", (it) => {
  ...
  it.effect("admits only one parallel reservation and persists the losing domain receipt", () =>
    Effect.gen(function* () {
      ...
      const contender = Effect.fn("parallelReserveContender")(function* (input: typeof firstInput) {
        ...
        return yield* Effect.result(engine.dispatchController(input));
      });
      const firstFiber = yield* contender(firstInput).pipe(Effect.forkChild);
      const secondFiber = yield* contender(secondInput).pipe(Effect.forkChild);
      yield* Deferred.await(bothReady);
      yield* Deferred.succeed(releaseBarrier, undefined);
      const results = yield* Effect.all([Fiber.join(firstFiber), Fiber.join(secondFiber)], {
  ...
  it.effect("commits one event and one receipt for a barriered identical command race", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      const engine = yield* AgentControlStageRunLeaseEngine;
      const projectId = ProjectId.make("lease-identical-command-race");
      const { stageRun } = yield* seedPrepared(projectId);
      const input = yield* resolvedReserveInput(stageRun, "lease-identical-command", 0, 1);
      const published = yield* Ref.make(0);
      const publicationFiber = yield* Stream.runForEach(engine.streamDomainEvents, () =>
      ...
      const bothReady = yield* Deferred.make<void>();
      const releaseBarrier = yield* Deferred.make<void>();
      const contender = Effect.fn("identicalCommandContender")(function* () {
        const arrived = yield* Ref.updateAndGet(arrivals, (count) => count + 1);
        if (arrived === 2) yield* Deferred.succeed(bothReady, undefined);
        yield* Deferred.await(releaseBarrier);
        return yield* engine.dispatchController(input);
      });
      const firstFiber = yield* contender().pipe(Effect.forkChild);
      const secondFiber = yield* contender().pipe(Effect.forkChild);
      yield* Deferred.await(bothReady);
      yield* Deferred.succeed(releaseBarrier, undefined);
  ...
  it.effect("never reuses a fence token across a barriered release-reserve race", () =>
    Effect.gen(function* () {
      const engine = yield* AgentControlStageRunLeaseEngine;
      const projectId = ProjectId.make("lease-release-reserve-race");
      const { task, stageRun } = yield* seedPrepared(projectId);
      const initial = yield* engine.dispatchController(
        yield* resolvedReserveInput(stageRun, "lease-race-initial", 0, 1),
      );
      assert.equal(initial._tag, "Accepted");
      if (initial._tag !== "Accepted") return;
      const release = {
        type: "agentControl.stageRunLease.releaseBeforeExecution" as const,
        commandId: CommandId.make("lease-race-release"),
        leaseId: initial.result.state.leaseId,
        projectId,
        taskId: task.taskId,
        stageRunId: stageRun.stageRunId,
        attemptId: stageRun.attemptId,
      ...
      const racedReserve = yield* resolvedReserveInput(stageRun, "lease-race-reserve", 2, 2);
      const arrivals = yield* Ref.make(0);
      const bothReady = yield* Deferred.make<void>();
      const releaseBarrier = yield* Deferred.make<void>();
      const awaitBarrier = Effect.gen(function* () {
        const arrived = yield* Ref.updateAndGet(arrivals, (count) => count + 1);
        if (arrived === 2) yield* Deferred.succeed(bothReady, undefined);
        yield* Deferred.await(releaseBarrier);
      });
      const releaseFiber = yield* awaitBarrier.pipe(
        Effect.andThen(engine.dispatchController(release)),
        Effect.forkChild,
      );
      const reserveFiber = yield* awaitBarrier.pipe(
        Effect.andThen(engine.dispatchController(racedReserve)),
        Effect.result,
        Effect.forkChild,
      );
      yield* Deferred.await(bothReady);
      yield* Deferred.succeed(releaseBarrier, undefined);
      const released = yield* Fiber.join(releaseFiber);
      const reserveRace = yield* Fiber.join(reserveFiber);
      assert.equal(released._tag, "Accepted");

      const finalReserve =
        reserveRace._tag === "Success" && reserveRace.success._tag === "Accepted"
          ? reserveRace.success
          : yield* engine.dispatchController(
              yield* resolvedReserveInput(stageRun, "lease-race-reserve-after", 2, 2),
            );
      assert.equal(finalReserve._tag, "Accepted");
      if (finalReserve._tag !== "Accepted") return;
      assert.equal(finalReserve.result.state.fenceToken, 2);
      assert.equal(finalReserve.result.state.revision, 3);

      const staleHolderFence = yield* engine.dispatchController({
        type: "agentControl.stageRunLease.renew",
        commandId: CommandId.make("lease-race-stale-fence"),
        leaseId: initial.result.state.leaseId,
        projectId,
        taskId: task.taskId,
        stageRunId: stageRun.stageRunId,
      ...
      assert.equal(staleHolderFence._tag, "Rejected");
      if (staleHolderFence._tag === "Rejected") {
        assert.equal(staleHolderFence.error.code, "fence-token-mismatch");
      }
    }),
  );
