...
const decodeVerificationReleaseStorage = Schema.decodeUnknownEffect(
  Schema.fromJsonString(AgentControlStageRunLeaseReleasedAfterVerificationPayloadStorage),
);
...
const decodeVerificationReleaseInput = Schema.decodeUnknownEffect(
  AgentControlStageRunLeaseReleasedAfterVerificationPayloadStorage,
);
  ...
  const decodeRows = (rows: ReadonlyArray<Record<string, unknown>>, operation: string) =>
    Effect.forEach(rows, (rawRow) =>
      Effect.gen(function* () {
        ...
        const payload = yield* row.type === "agentControl.stageRunLease.releasedAfterVerification"
          ? decodeVerificationReleaseStorage(row.payload)
        ...
        Effect.gen(function* () {
          ...
          return yield* Effect.forEach(
            ...
            (draft, index) =>
              Effect.gen(function* () {
                const payloadInput =
                  draft.type === "agentControl.stageRunLease.releasedAfterVerification"
                    ? yield* decodeVerificationReleaseInput(draft.payload).pipe(
                        Effect.mapError((cause) =>
                          decodeError(
                            "AgentControlStageRunLeaseEventStore.append:payload-storage",
                            cause,
                          ),
                        ),
                      )
