...
import { loadEpicTaskExecutionAuthority } from "../executionAuthority.ts";
...
const make = Effect.gen(function* () {
  ...
  const useValidatedTask = <A, E, R>(
    ...
    Effect.gen(function* () {
      ...
      const executionTask = yield* loadEpicTaskExecutionAuthority(sql, task);
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          const callbackFiber = yield* use(executionTask, gate).pipe(
