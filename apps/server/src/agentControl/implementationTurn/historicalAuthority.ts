...
export const AgentControlImplementationHistoricalAuthorityReason = Schema.Literals([
  "history-missing",
  "history-divergent",
  "history-undecodable",
  "projection-missing",
...
export type AgentControlImplementationHistoricalAuthorityReason =
  typeof AgentControlImplementationHistoricalAuthorityReason.Type;

export class AgentControlImplementationHistoricalAuthorityError extends Schema.TaggedErrorClass<AgentControlImplementationHistoricalAuthorityError>()(
  "AgentControlImplementationHistoricalAuthorityError",
  {
    operation: Schema.String,
    reason: AgentControlImplementationHistoricalAuthorityReason,
...
const historyError = (
  ...
  new AgentControlImplementationHistoricalAuthorityError({
    operation,
    reason,
...
const decodeStoredEvent = Effect.fn("decodeStoredImplementationAuthorityEvent")(function* <A>(
...
export const loadAgentControlImplementationTaskAuthorityInTransaction = Effect.fn(
  ...
  const events = yield* Effect.forEach(rawEvents, (row) =>
    decodeStoredEvent(row, decodeTaskEvent, encodeTaskEvent, "task-history-event"),
  );
  ...
  const projection = yield* decodeAgentControlTaskProjectionRow(
    ...
    "implementation-task-projection",
  ).pipe(
...
export const loadAgentControlImplementationWorktreeAuthorityInTransaction = Effect.fn(
  ...
  const events = yield* Effect.forEach(rawEvents, (row) =>
    decodeStoredEvent(row, decodeWorktreeEvent, encodeWorktreeEvent, "worktree-history-event"),
  );
  const selectedIndex =
    target === undefined
      ? events.length - 1
  ...
  const projection = yield* decodeAgentControlWorktreeProjectionRow(
    ...
    "implementation-worktree-projection",
  ).pipe(
    Effect.mapError((cause) =>
