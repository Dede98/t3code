...
it.effect(
  "advances through two explicit strict repair candidates with durable replay identities",
  () =>
    Effect.gen(function* () {
      ...
      const repairLayer = EpicReviewRepairLive.pipe(
        Layer.provide(Layer.merge(Layer.succeed(AgentControlPolicyService, policy), orchestration)),
      );
      ...
      const checkpointCommit = yield* git(
        ["commit-tree", checkpointTree, "-p", reviewedCommitSha, "-m", "authorized checkpoint"],
        secondPath,
      );
      const checkpointRef = "refs/t3/checkpoints/review-repair/turn/1";
      yield* git(["update-ref", checkpointRef, checkpointCommit]);
      yield* git(["reset", "--hard", reviewedCommitSha], secondPath);
      yield* Effect.tryPromise(() =>
        NodeFSP.writeFile(NodePath.join(secondPath, "later-unowned.txt"), "must not be accepted\n"),
      );
