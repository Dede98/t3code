...
it.layer(NodeServices.layer)("server settings", (it) => {
  ...
  it.effect("rolls back external MCP header secrets when the settings file commit fails", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      let failRename = false;
      let settingsPathToFail: string | undefined;
      ...
      const serverId = ExternalMcpServerId.make("mcp_write_failure");
      const server = (value: string) => ({
        url: "https://rollback.example/mcp",
        enabled: true,
        headers: [{ name: "Authorization", value, sensitive: true }],
      });
      const settingsLayer = makeServerSettingsLayer().pipe(
        Layer.provideMerge(Layer.succeed(FileSystem.FileSystem, failingFileSystem)),
      );
      ...
      yield* Effect.gen(function* () {
        ...
        settingsPathToFail = (yield* ServerConfig.ServerConfig).settingsPath;
        yield* serverSettings.updateSettings({
          externalMcpServers: { [serverId]: server("Bearer kept") },
        });

        failRename = true;
        const failedUpdate = yield* serverSettings
          .updateSettings({ externalMcpServers: { [serverId]: server("Bearer new") } })
          .pipe(Effect.result);
        assert.equal(failedUpdate._tag, "Failure");
        assert.equal(
          (yield* serverSettings.getSettings).externalMcpServers[serverId]?.headers[0]?.value,
          "Bearer kept",
        );

        const failedRemoval = yield* serverSettings
          .updateSettings({ externalMcpServers: {} })
          .pipe(Effect.result);
        assert.equal(failedRemoval._tag, "Failure");
        assert.equal(
          (yield* serverSettings.getSettings).externalMcpServers[serverId]?.headers[0]?.value,
          "Bearer kept",
        );
