    ...
    Effect.gen(function* () {
      ...
      const agentControlEpic = Option.getOrElse(epicService, () =>
        AgentControlEpic.of({
          ...
          requestReviewRework: unavailableEpic,
      ...
      const coalesceShellEvents = (
        ...
        Effect.gen(function* () {
          const externallyVisibleEvents = events.filter(
            (event) => event.type !== "thread.verification-result-fragment-captured",
          );
      ...
      return WsRpcGroup.of({
        ...
        [AGENT_CONTROL_TASK_RPC_METHODS.getTask]: (input) =>
          observeRpcEffect(
            AGENT_CONTROL_TASK_RPC_METHODS.getTask,
            agentControlTasks.getTask(input),
            { "rpc.aggregate": "agent-control-task" },
          ),
        [AGENT_CONTROL_TASK_RPC_METHODS.listTasks]: (input) =>
          observeRpcEffect(
            AGENT_CONTROL_TASK_RPC_METHODS.listTasks,
            agentControlTasks.listTasks(input),
            { "rpc.aggregate": "agent-control-task" },
          ),
        [AGENT_CONTROL_TASK_RPC_METHODS.reconcileOnce]: (input) =>
          observeRpcEffect(
            AGENT_CONTROL_TASK_RPC_METHODS.reconcileOnce,
            agentControlTasks.reconcileOnce(input),
            { "rpc.aggregate": "agent-control-task" },
          ),
        [AGENT_CONTROL_TASK_RPC_METHODS.getReactorStatus]: (input) =>
          observeRpcEffect(
        ...
        [AGENT_CONTROL_WORKTREE_RPC_METHODS.getReservation]: (input) =>
          observeRpcEffect(
            AGENT_CONTROL_WORKTREE_RPC_METHODS.getReservation,
            agentControlWorktrees.getReservation(input),
            { "rpc.aggregate": "agent-control-worktree" },
          ),
        [AGENT_CONTROL_WORKTREE_RPC_METHODS.listReservations]: (input) =>
          observeRpcEffect(
        ...
        [AGENT_CONTROL_EPIC_RPC_METHODS.requestReviewRework]: (input) =>
          observeRpcEffect(
            AGENT_CONTROL_EPIC_RPC_METHODS.requestReviewRework,
            agentControlEpic.requestReviewRework(input),
            { "rpc.aggregate": "agent-control" },
          ),
        ...
        [WS_METHODS.subscribeProviderUsage]: (_input) =>
          observeRpcStreamEffect(
            ...
            Effect.succeed(
              providerUsage.stream ??
                Stream.unwrap(
                  providerUsage.subscribeEvents.pipe(Effect.map(Stream.fromSubscription)),
                ),
            ),
            { "rpc.aggregate": "server" },
          ),
