...
it.layer(NodeSqliteClient.layer({ filename: ":memory:" }))(
  "050_ProjectionThreadsActiveOrderKey",
  (it) => {
    it.effect("migrates old threads without changing their timestamps or assigning an order", () =>
      ...
      `;
        ...
        const branchLinks = yield* sql<{ readonly branchPullRequest: string | null }>`
        SELECT branch_pull_request_json AS "branchPullRequest"
        FROM projection_threads WHERE thread_id = 'thread-1'
      `;
        assert.deepEqual(branchLinks, [{ branchPullRequest: null }]);
        yield* runMigrations({ toMigrationInclusive: 50 });
        const migrationNames = yield* sql<{ readonly name: string }>`
        SELECT name FROM effect_sql_migrations
        WHERE migration_id IN (33, 48, 49, 50) ORDER BY migration_id
      `;
        assert.deepEqual(
