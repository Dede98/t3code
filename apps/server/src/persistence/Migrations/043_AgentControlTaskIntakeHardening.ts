...
/**
 * Hardens task intake without rewriting reconstructible task projections.
 * Creating the source-number unique index intentionally fails if an existing
 * projection is ambiguous; migration must never pick a winner.
 */
export default Effect.gen(function* () {
  const sql = yield* SqlClient.SqlClient;

  yield* sql`
    CREATE TABLE agent_control_events_rebuild_043 (
  ...
  `;
  yield* sql`
    INSERT INTO agent_control_events_rebuild_043 (
  ...
  yield* sql`ALTER TABLE agent_control_events_rebuild_043 RENAME TO agent_control_events`;
  yield* sql`
  ...
  yield* sql`
    CREATE TABLE agent_control_command_receipts_rebuild_043 (
  ...
  `;
  yield* sql`
    INSERT INTO agent_control_command_receipts_rebuild_043 (
  ...
  yield* sql`
    ALTER TABLE agent_control_command_receipts_rebuild_043
  ...
  yield* sql`
    DROP INDEX idx_agent_control_task_issue_number
  `;
  yield* sql`
  ...
  yield* sql`
    ...
    ON agent_control_task_states(project_id, issue_number, task_id)
  `;

  yield* sql`
    CREATE TABLE agent_control_task_reconcile_states (
      project_id TEXT PRIMARY KEY,
      target_sequence INTEGER NOT NULL CHECK (target_sequence >= 1),
      last_completed_sequence INTEGER NOT NULL CHECK (
        last_completed_sequence >= 0 AND last_completed_sequence <= target_sequence
      ),
      revision INTEGER NOT NULL CHECK (revision >= 1),
      status TEXT NOT NULL CHECK (
        status IN ('reconciling', 'completed', 'recovery-required')
      ),
      updated_at TEXT NOT NULL
    )
  `;
  yield* sql`
    CREATE INDEX idx_agent_control_task_reconcile_status
    ON agent_control_task_reconcile_states(status, target_sequence, project_id)
  `;
});
