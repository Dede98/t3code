...
const ProjectionThreadDbRowSchema = ProjectionThread.mapFields(
  Struct.assign({
    ...
    agentControl: Schema.NullOr(Schema.fromJsonString(AgentControlThreadBinding)),
...
const ProjectionThreadRuntimeContextDbRowSchema = Schema.Struct({
  id: ThreadId,
  title: Schema.String,
  boundProviderInstanceId: Schema.NullOr(ProviderInstanceId),
    ...
    execute: () =>
      sql`
        SELECT
          ...
          agent_control_json AS "agentControl",
    ...
    execute: ({ threadId }) =>
      sql`
        SELECT
          ...
          threads.title,
          runtime.provider_instance_id AS "boundProviderInstanceId",
          sessions.thread_id AS "threadId",
          sessions.status,
          sessions.provider_name AS "providerName",
          sessions.provider_instance_id AS "providerInstanceId",
      ...
      `.pipe(
        Effect.map((rows) =>
          rows.map((row) => ({
            id: row.id,
            title: row.title,
            boundProviderInstanceId: row.boundProviderInstanceId,
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                ...(row.agentControl !== null ? { agentControl: row.agentControl } : {}),
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        worktreePath: threadRow.value.worktreePath,
        ...(threadRow.value.agentControl !== null
          ? { agentControl: threadRow.value.agentControl }
          : {}),
