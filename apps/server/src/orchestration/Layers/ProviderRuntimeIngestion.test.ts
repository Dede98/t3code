...
function createProviderServiceHarness(
  boundProviderInstanceId: ProviderInstanceId = CODEX_INSTANCE_ID,
) {
  ...
  const directEvents = Effect.runSync(PubSub.unbounded<ProviderRuntimeEvent>());
  ...
  const service: ProviderServiceShape = {
    ...
    subscribeEvents: PubSub.subscribe(directEvents),
    get streamEvents() {
  ...
  const emit = (event: LegacyProviderRuntimeEvent): void => {
    ...
    Effect.runSync(PubSub.publish(directEvents, normalized));
    Effect.runSync(PubSub.publish(runtimeEventPubSub, { events: [normalized] }));
  };
...
describe("ProviderRuntimeIngestion", () => {
  ...
  async function createHarness(options?: {
    ...
    const initialProviderInstanceId =
      options?.providerInstanceId ?? ProviderInstanceId.make(initialProvider);
    ...
    const providerSessionDirectory = await testRuntime.runPromise(
      Effect.service(ProviderSessionDirectory),
    );
    const providerSessionRuntimeRepository = await testRuntime.runPromise(
    ...
    return {
      ...
      upsertProviderSession: (binding: ProviderRuntimeBinding) =>
        testRuntime.runPromise(providerSessionDirectory.upsert(binding)),
      drain,
    };
  }
  ...
  it("accepts a targeted turn.completed when no turn is active", async () => {
    const harness = await createHarness({ provider: ProviderDriverKind.make("claudeAgent") });
    const seededAt = "2026-01-01T00:00:00.000Z";
  ...
  it("buffers assistant deltas with bounded lifecycle and authority queries per event", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    // One lifecycle read plus the concrete binding and AgentControl authority reads.
    expect(harness.sqlCount() - before).toBe(eventCount * 3);
