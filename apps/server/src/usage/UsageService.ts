...
export const make = Effect.gen(function* () {
  ...
  const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(function* (
    ...
    const sources = yield* Effect.forEach(instances, (instance) =>
      Effect.gen(function* () {
        ...
        const sourceKey = source.provider + "\0" + source.transcriptDirectory;
        const previous = sourceCache.get(sourceKey);
        // Keep canonical paths and fingerprints stable after root cleanup, including aliases.
        const transcriptDirectory = yield* fileSystem
          .realPath(source.transcriptDirectory)
          .pipe(Effect.orElseSucceed(() => previous?.dir ?? source.transcriptDirectory));
        const currentVolumeId = yield* Effect.promise(() =>
          readDirectoryVolumeId(transcriptDirectory),
        );
        const hasRetainedHistory = fileCache
          ...
          .some(
            ([filePath, entry]) =>
              ...
              isWithinDirectory(filePath, transcriptDirectory),
          );
        const volumeId =
          previous?.dir === transcriptDirectory && (hasRetainedHistory || !currentVolumeId)
            ? previous.volumeId || currentVolumeId
            : currentVolumeId;
        if (previous?.dir !== transcriptDirectory || previous.volumeId !== volumeId) {
          sourceCache.set(sourceKey, { dir: transcriptDirectory, volumeId });
          cacheDirty = true;
        }
        volumes.set(source.provider + "\0" + transcriptDirectory, volumeId);
        return { usageHistorySource: { ...source, transcriptDirectory } };
      }),
    );
    return collectUsageTranscriptSources(sources).map((source) => ({
      ...source,
      volumeId: volumes.get(source.provider + "\0" + source.dir) ?? "",
    }));
  });
  ...
  const collectDirs = Effect.fn("UsageService.collectDirs")(function* (
    ...
    const dirs = yield* resolveTranscriptDirs(retentionCutoffMs);
    const scanned: ScannedDir[] = [];
    for (const { sourceId, provider, dir, volumeId, fileName } of dirs) {
      const exists = yield* fileSystem
        .exists(dir)
        .pipe(Effect.catchCause(() => Effect.succeed(false)));
      if (!exists) {
        scanned.push({ sourceId, provider, dir, volumeId, files: null });
        continue;
      }
      ...
      scanned.push({ sourceId, provider, dir, volumeId, files: parsedFiles });
    }
    return scanned;
  });
  ...
  const scanSummary = Effect.fn("UsageService.scanSummary")(function* (
    ...
    const [, scannedDirs] = yield* Effect.all(
      [ensureRates(false), collectDirs(windowStartMs, retentionCutoffMs)],
      { concurrency: 2 },
    );
    ...
    for (const { sourceId, provider, dir, volumeId, files } of scannedDirs) {
