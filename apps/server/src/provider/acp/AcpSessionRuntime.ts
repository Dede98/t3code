  ...
  Effect.gen(function* () {
    ...
    const acpContext = yield* Layer.build(
      EffectAcpClient.layerChildProcess(child, {
        ...
        onTermination: (cause) =>
          Effect.gen(function* () {
            yield* options.onTransportTermination?.(cause) ?? Effect.void;
            const failure = cause.reasons.find(Cause.isFailReason);
            yield* recordTermination(
              failure && Cause.isFailReason(failure)
                ? failure.error
                : new EffectAcpErrors.AcpTransportError({
                    detail: "ACP transport terminated",
                    cause,
                  }),
            );
          }),
          ...
          Effect.acquireUseRelease(
            promptDispatchSemaphore.withPermit(
              Effect.gen(function* () {
                      ...
                      Effect.flatMap((done) =>
                          ...
                          : Exit.isFailure(exit)
                            ...
                            : Deferred.die(
                                outgoingAck,
                                new Error(
                                  "ACP prompt completed before its outgoing acknowledgement.",
                                ),
                              ).pipe(Effect.asVoid),
                      ),
                    ),
                  ),
                  Effect.forkIn(runtimeScope, { startImmediately: true, uninterruptible: false }),
                );
                ...
                let reported = false;
                const reportNativeInvocation = Effect.suspend(() => {
                  if (reported) return Effect.void;
                  reported = true;
                  return promptOptions?.nativeInvocationStarted?.() ?? Effect.void;
                }).pipe(Effect.uninterruptible);
                return { ...active, outgoingAck, reportNativeInvocation };
              }),
            ),
            (activePrompt) =>
              Effect.gen(function* () {
                const evidence = yield* Deferred.await(activePrompt.outgoingAck);
                if (evidence.method !== "session/prompt" || evidence.requestId.length === 0) {
                  return yield* Effect.die(
                    new Error("ACP prompt received mismatched outgoing acknowledgement."),
                  );
                }
                yield* activePrompt.reportNativeInvocation;
                return yield* Fiber.join(activePrompt.fiber);
              }).pipe(
                Effect.onError(() =>
                  Deferred.poll(activePrompt.outgoingAck).pipe(
                    Effect.flatMap((ack) =>
                      Option.isSome(ack)
                        ? Effect.exit(ack.value).pipe(
                            Effect.flatMap((exit) =>
                              Exit.isSuccess(exit)
                                ? activePrompt.reportNativeInvocation
                                : Effect.void,
                            ),
                          )
                        : Effect.void,
    ...
    if (options.onRuntimeIdentity !== undefined) {
      yield* options.onRuntimeIdentity({
        ...
        activePromptFiberRef: activePromptRef,
        sessionLoadGateRef,
        childProcess: child,
        childPid: child.pid,
        snapshot: Effect.gen(function* () {
          const activePromptFiber = yield* Ref.get(activePromptRef);
