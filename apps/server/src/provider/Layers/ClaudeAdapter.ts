...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    // These wire-only informational notices are absent from the SDK union.
    // The work log already shows their underlying git/gh tool calls, so avoid
    // surfacing duplicate unknown-subtype warnings. Keep
    // `background_tasks_changed` in the typed switch below: the pinned SDK
    // exposes it and T3 uses it to reconcile the background-task roster.
