  ...
  return Effect.gen(function* () {
    ...
    const startSession: GrokAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const acpRuntimeLoggers = acpNativeLoggers;
          ...
          if ((grokModelSelection?.options?.length ?? 0) > 0) {
            return yield* new ProviderAdapterValidationError({
              provider: PROVIDER,
              operation: "startSession",
              issue: "Grok ACP only applies the reasoningEffort model option.",
            });
          }
    ...
    const prepareTurn: NonNullable<GrokAdapterShape["prepareTurn"]> = Effect.fn("prepareTurn")(
      function* (input) {
        ...
        if (selection.options?.some((option) => option.id !== "reasoningEffort")) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "prepareTurn",
            issue: "Grok ACP only applies the reasoningEffort model option.",
        ...
        const effectiveReasoningEffort = normalizeGrokReasoningEffort(requestedReasoningEffort);
        if (selection.options?.length && effectiveReasoningEffort === undefined) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "prepareTurn",
            issue: "Grok reasoning effort was not applied.",
          });
        }
        ...
        const effectiveModelSelection = {
          ...
          ...(effectiveReasoningEffort === undefined
            ? {}
            : { options: [{ id: "reasoningEffort", value: effectiveReasoningEffort }] }),
        };
