...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("maps the catalog default effort to the SDK-supported max value", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
        ...
        modelSelection: {
          ...
          model: "claude-synthetic-normalized",
        },
        runtimeMode: "full-access",
      });
  ...
  it.effect("preserves an SDK-supported xhigh effort", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
  ...
  it.effect(
    "falls back to default effort when unsupported max is requested for a restricted model",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
  ...
  it.effect("ignores adaptive effort for a thinking-only model", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
  ...
  it.effect("forwards Claude thinking toggle into SDK settings for a thinking-only model", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
  ...
  it.effect("ignores Claude thinking toggle for models without a thinking toggle", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
  ...
  it.effect("settles a synthetic background turn when interrupt produces no Claude result", () => {
    ...
    return Effect.gen(function* () {
      ...
      // A parent-conversation assistant message arriving with no active turn
      // opens a synthetic turn. (Subagent-owned snapshots carry
      // parent_tool_use_id and are consumed without opening one.)
      harness.query.emit({
        type: "assistant",
        session_id: "sdk-session-background-interrupt",
      ...
      assert.equal(settledSessions.length, 0);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect.each([
    ...
    ({ homePath, inherited }) => {
      ...
      return Effect.gen(function* () {
        ...
        const expectedConfigDir = NodePath.resolve((inherited ?? homePath).trim());
  ...
  it.effect("task.started carries model/effort; subagent snapshots refine the model", () => {
    ...
    return Effect.gen(function* () {
      ...
      const updated = taskEvents[1];
      assert.equal(updated?.type, "task.updated");
      if (updated?.type === "task.updated")
        assert.equal(updated.payload.model, SYNTHETIC_SUBAGENT_MODEL);
      const progress = taskEvents[2];
  ...
  it.effect("maps Claude task state patches and normalizes killed tasks to cancelled", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      if (updatedEvent?.type === "task.updated") {
        assert.deepEqual(updatedEvent.payload, {
          ...
          endedAt: new Date(1_784_107_695_421).toISOString(),
          totalPausedMs: 250,
        });
      }
      assert.equal(
  ...
  it.effect("correlates Claude subagent task metadata onto the task.started row", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const runtimeEventsFiber = yield* adapter.streamEvents.pipe(
      ...
      if (started?.type === "task.started") {
        assert.equal(started.payload.toolUseId, "tool-agent-model");
        assert.equal(started.payload.role, "code-reviewer");
        assert.equal(started.payload.title, "Review the migration");
        assert.equal(started.payload.taskType, "local_agent");
        assert.equal(started.payload.model, "opus");
      }
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
  ...
  it.effect("consumes undeclared and UX-internal system subtypes without warning rows", () => {
    ...
    return Effect.gen(function* () {
      ...
      const runtimeEventsFiber = yield* adapter.streamEvents.pipe(
        Stream.takeUntil(
          (event) =>
            ...
            event.payload.reason === "control_request_api_retry:2/4",
        ),
        Stream.runCollect,
        Effect.forkChild,
      );
      ...
      harness.query.emit({
        ...
        trigger: "refusal",
        direction: "retry",
        original_model: "claude-synthetic-native-xhigh",
      ...
      const controlRetry = runtimeEvents.find(
        (event) =>
          event.type === "session.state.changed" &&
          event.payload.reason === "control_request_api_retry:2/4",
      );
      assert.equal(controlRetry?.type, "session.state.changed");
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("acceptForSession returns session-scoped permission updates", () => {
    ...
    return Effect.gen(function* () {
      ...
      const mcpPermissionPromise = canUseTool(
        ...
        { title: "hello" },
        {
          requestId: "permission-request-session-mcp",
          signal: new AbortController().signal,
          suggestions: [],
      ...
      const bashPermissionPromise = canUseTool(
        ...
        { command: "git status" },
        {
          requestId: "permission-request-session-bash",
          signal: new AbortController().signal,
          suggestions: [
            {
              type: "addRules",
    ...
    return Effect.gen(function* () {
      ...
      const permissionPromise = canUseTool(
        ...
        },
        {
          ...
          requestId: "permission-request-question-pre-aborted",
        },
      );
  ...
  it.effect("stopping a session settles pending user-input waits", () => {
    ...
    return Effect.gen(function* () {
      ...
      const permissionPromise = canUseTool(
        ...
        {
          requestId: "permission-request-question-stop",
          signal: new AbortController().signal,
