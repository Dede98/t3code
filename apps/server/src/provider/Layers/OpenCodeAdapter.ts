  ...
  return Effect.gen(function* () {
    ...
    const startSession: OpenCodeAdapterShape["startSession"] = Effect.fn("startSession")(
      function* (input) {
        ...
        return attestProviderSessionNativeConfiguration(context.session, null);
      },
    );

    const preparedEntries = new WeakMap<
      object,
      import("../Services/ProviderAdapter.ts").ProviderAdapterTurnEntry
    >();
      ...
      function* (input) {
        ...
        const selection =
          ...
          (context.session.model
            ...
            : undefined);
        const model = parseOpenCodeModelSlug(selection?.model);
        if (
          !selection ||
          selection.instanceId !== boundInstanceId ||
          !model ||
          selection.options?.some((option) => option.id !== "agent" && option.id !== "variant")
        ) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "prepareTurn",
            issue: "OpenCode turn requires a supported model configuration for this instance.",
          });
        }
        const agent =
          getModelSelectionStringOptionValue(selection, "agent") ??
          (input.interactionMode === "plan" ? "plan" : undefined);
        const variant = getModelSelectionStringOptionValue(selection, "variant");
        const options = [
          ...(agent ? [{ id: "agent", value: agent }] : []),
          ...(variant ? [{ id: "variant", value: variant }] : []),
        ];
        const effective = {
        ...
        const preparedInput = { ...input, modelSelection: effective };
        return {
          attestation: attestProviderNativeTurnConfiguration(effective),
          invoke: (entry) => {
            preparedEntries.set(preparedInput, entry);
            return sendTurn(preparedInput).pipe(
              Effect.ensuring(Effect.sync(() => preparedEntries.delete(preparedInput))),
            );
          },
        };
      },
    ...
    const sendTurn: OpenCodeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
      ...
      return yield* context.promptSemaphore.withPermit(
        Effect.gen(function* () {
          ...
          const entry = preparedEntries.get(input);
          yield* entry?.adapterEntered() ?? Effect.void;
          const invokeNative = () =>
          ...
          const promptEffect = (entry ? entry.startExternal(invokeNative) : invokeNative()).pipe(
            Effect.timeout("10 seconds"),
            Effect.catchTags({
