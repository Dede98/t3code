...
/** Canonicalizes the deepest existing ancestor while preserving a missing tail for new files. */
const realPathWithMissingTail = Effect.fn("AntigravityAdapter.realPathWithMissingTail")(function* (
  ...
  while (true) {
    ...
    if (Option.isSome(real)) return path.join(real.value, ...missingTail);
...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    // Follow every existing symlink while retaining missing path segments for a new file.
    const real = yield* realPathWithMissingTail(input.fileSystem, path, resolved);
    const roots = yield* Effect.forEach(input.allowedRoots, (root) =>
      input.fileSystem.realPath(root).pipe(Effect.orElseSucceed(() => path.resolve(root))),
    );
