...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    ...
    it.effect.each([true, false])(
      "matches the Claude runtime directory precedence with explicit config: %s",
      (explicitConfig) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const claudeHomePath = yield* makeTempDir("t3code-claude-legacy-");
          const codexHomePath = yield* makeTempDir("t3code-codex-home-");
          const selectedHome = yield* makeTempDir("t3code-claude-selected-");
          const workspace = yield* makeTempDir("t3code-selected-workspace-");
          const ignoredWorkspace = yield* makeTempDir("t3code-ignored-workspace-");
          for (const [home, cwd] of [
            [selectedHome, workspace],
            [claudeHomePath, ignoredWorkspace],
          ] as const) {
            yield* writeTranscript({
              filePath: path.join(home, "projects", "project", "session.jsonl"),
            ...
            providerInstances: {
              [ProviderInstanceId.make("claudeAgent")]: {
                ...
                config: {
                  ...
                  configDirPath: explicitConfig ? selectedHome : "",
                },
                environment: [
                  {
                    name: "CLAUDE_CONFIG_DIR",
                    value: explicitConfig ? claudeHomePath : selectedHome,
                    sensitive: false,
                  },
                ],
              },
    ...
    it.effect("excludes sandboxes reached through a symlink into the worktrees dir", () =>
      Effect.gen(function* () {
        ...
        const configAlias = path.join(linkParent, "config");
        yield* fileSystem.symlink(configBaseDir, configAlias);
        yield* writeTranscript({
          filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
