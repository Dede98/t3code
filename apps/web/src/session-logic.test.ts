...
describe("deriveWorkLogEntries", () => {
  ...
  it("omits task.started but shows task.progress and task.completed", () => {
    ...
    // Quiet timeline: a task.started row without agent identity is not a
    // standalone narrative row (agent spawns get a single CTA row instead).
    const entries = deriveWorkLogEntries(activities);
  ...
  it("keeps terminal task state patches out of the work log", () => {
    // task.updated is fold input for the Agents surface, not narrative: a
    // status patch never earns its own timeline row.
    const entries = deriveWorkLogEntries([
      makeActivity({
        id: "task-updated",
        createdAt: "2026-02-23T00:00:02.000Z",
        ...
        tone: "info",
        payload: { taskId: "task-1", status: "cancelled" },
      }),
    ]);
