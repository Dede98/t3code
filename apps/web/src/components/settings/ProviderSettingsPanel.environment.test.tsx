...
vi.mock("../../localApi", () => ({
  readLocalApi: () => ({ dialogs: { confirm: dialogs.confirm } }),
  ensureLocalApi: () => ({ dialogs: { confirm: dialogs.confirm } }),
}));
...
describe("EnvironmentProviderSettings routing", () => {
  ...
  it("routes confirmed Claude cross-account continuation to the selected environment", async () => {
    const panel = renderPanel();
    ...
    expect(continuationSwitch).not.toBeNull();
